...
- (void)insertTabWithImageDic:(NSDictionary *)dict atIndex:(NSUInteger)index MaxTag:(NSUInteger)maxTag
{
    ...
    UIImage *defaultImage = [GeneralUIUse GetImageLocal:[dict objectForKey:TabBarDefault] BundleResource:@"ZYYLibBundle"];
    UIImage *seletedImage = [GeneralUIUse GetImageLocal:[dict objectForKey:TabBarSeleted] BundleResource:@"ZYYLibBundle"];
    ...
    btn.titleEdgeInsets = UIEdgeInsetsMake(35, -btn.imageView.frame.size.width, -btn.imageView.frame.size.height, 0);
...
- (void)setTitleToNew:(NSInteger)which Title:(NSString *)titleStr ImageNor:(UIImage *)imageNor ImageSelected:(UIImage *)imageSelected {
    if (which < MAXNEWPOINT && [self.buttons count] > which) {
        ...
        NSDictionary *initInfo = FCJsonLocal(@"TabBarSetting.json");
        if (!initInfo) {
            
        }
        NSArray *tabBarItem = [initInfo objectForKey:@"tabBarTag"];
        
        NSDictionary * dic = [tabBarItem objectAtIndex:which];
        
        UIButton *btn = [self.buttons objectAtIndex:which];
        
        UIImage *defaultImage = imageNor;
        UIImage *seletedImage = imageSelected;
        ...
        [btn.titleLabel setFont:[UIFont systemFontOfSize:12]];
        [btn setTitle:ML(titleStr) forState:UIControlStateNormal];
        [btn setTitle:ML(titleStr) forState:UIControlStateSelected];
