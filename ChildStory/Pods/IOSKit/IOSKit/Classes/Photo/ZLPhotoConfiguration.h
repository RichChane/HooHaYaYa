...
/**
 最大选择视频数 默认1张，受maxSelectCount限制
 */
@property (nonatomic, assign) NSInteger maxSelectVideoCount;
...
/**
 是否允许删除，只有选中预览大图时才需要删除，默认NO
 */
@property (nonatomic, assign) BOOL allowDelEdit;
...
@property (nonatomic, assign) NSInteger bottomHeight;           // 底部高度
@property (nonatomic, assign) NSInteger bottomImageSize;           // 底部图片大小（长宽）

@property (nonatomic, strong) UIImage *leftImage;       // 返回图标
@property (nonatomic, strong) UIImage *rightImage;       // 右上角图标图标
