...
- (ZLPreviewView *)previewView
{
    if (!_previewView) {
        ...
        _previewView.playIcon = _playIcon;
...
- (void)setModel:(ZLPhotoModel *)model
{
    ...
    self.previewView.singleTapCallBack = self.singleTapCallBack;
    
    self.previewView.longPressCallBack = self.longPressCallBack;
...
{
    if (!_netVideoView) {
        _netVideoView = [[ZLPreviewNetVideo alloc] initWithFrame:self.bounds Icon:_playIcon];
...
- (void)setModel:(ZLPhotoModel *)model
{
    ...
    switch (model.type) {
        ...
        case ZLAssetMediaTypeNetImage: {
            ...
            self.imageGifView.longPressCallBack = self.longPressCallBack;
        ...
        case ZLAssetMediaTypeNetVideo: {
            ...
            self.netVideoView.longPressCallBack = self.longPressCallBack;
        }
            break;
...
{
    if (self.model.type == ZLAssetMediaTypeGif) {
        if ([self.imageGifView.image_View.image isKindOfClass:NSClassFromString(@"_UIAnimatedImage")]) {
...
- (UIImage *)image
{
    if (self.model.type == ZLAssetMediaTypeImage ||
        self.model.type == ZLAssetMediaTypeGif ||
        self.model.type == ZLAssetMediaTypeNetImage) {
        return self.imageGifView.image_View.image;
    }
    return nil;
}
...
- (void)layoutSubviews
{
    ...
    [self resetScale];
    if (self.loadOK) {
        [self resetSubviewSize:self.asset?:self.image_View.image];
    }
}
...
- (void)initUI
{
    ...
    [self.containerView addSubview:self.image_View];
...
- (void)loadGifImage:(PHAsset *)asset
{
    ...
    [ZLPhotoManager requestOriginalImageDataForAsset:asset completion:^(NSData *data, NSDictionary *info) {
        ...
        if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue]) {
            strongSelf.image_View.image = [ZLPhotoManager transformToGifImageWithData:data];
...
- (void)loadNormalImage:(PHAsset *)asset
{
    ...
    self.imageRequestID = [ZLPhotoManager requestImageForAsset:asset size:size completion:^(UIImage *image, NSDictionary *info) {
        zl_strongify(weakSelf);
        strongSelf.image_View.image = image;
        [strongSelf resetSubviewSize:asset];
        if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue]) {
            [strongSelf.indicator stopAnimating];
...
- (void)loadImage:(id)obj
{
    ...
    if ([obj isKindOfClass:UIImage.class]) {
        self.image_View.image = obj;
        [self resetSubviewSize:obj];
    } else {
        [self.indicator startAnimating];
        zl_weakify(self);
        [self.image_View sd_setImageWithURL:obj placeholderImage:nil options:SDWebImageLowPriority completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            zl_strongify(weakSelf);
            [strongSelf.indicator stopAnimating];
            if (error) {
                ShowToastLong(ML(@"图片加载失败"));
            } else {
                strongSelf.loadOK = YES;
...
- (void)resetSubviewSize:(id)obj
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self.image_View.frame = self.containerView.bounds;
...
- (instancetype)initWithFrame:(CGRect)frame Icon:(UIImage *)playIcon
{
    self = [super initWithFrame:frame];
    if (self) {
        [self initUI];
...
- (void)loadNetVideo:(NSURL *)url
{
    ...
    NSString *flieUrl = nil;
    
    NSString *fileName = nil;
    NSArray *arr = [url.absoluteString componentsSeparatedByString:@"/product_video/"];
    if (arr && [arr count] == 2) {
        fileName = [@"product_video/"stringByAppendingString:arr[1]];
    }
    
    if (fileName && ![fileName hasSuffix:@".mp4"]) {
        fileName = [fileName stringByAppendingString:@".mp4"];
    }
    
    // 先把数据下载到本地
    if ([FileUseOther OperatingIsInFile:fileName]) {
        flieUrl = [FileUseOther OperatingRouteFile:fileName];
        [self loadVideo:flieUrl];
    } else {
        zl_weakify(self);
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if (data) {
                ...
                [strongSelf loadVideo:[FileUseOther OperatingRouteFile:fileName]];
            }
        });
    }
    
}
- (void)loadVideo:(NSString *)fileStr {
    dispatch_async(dispatch_get_main_queue(), ^{
        // 回到主线程进行UI操作
        self.playBtn.hidden = NO;
        [self.indicator stopAnimating];
        if (fileStr) {
            AVPlayer *player = [AVPlayer playerWithURL:[NSURL fileURLWithPath:fileStr]];
...
{
    if (_observerIsRemoved) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playFinished:) name:AVPlayerItemDidPlayToEndTimeNotification object:self.playLayer.player.currentItem];
        [self.playLayer.player.currentItem addObserver:self forKeyPath:@"playbackBufferEmpty" options:NSKeyValueObservingOptionNew context:nil];
        [self.playLayer.player.currentItem addObserver:self forKeyPath:@"playbackLikelyToKeepUp" options:NSKeyValueObservingOptionNew context:nil];
