...
- (void)layoutSubviews
{
    ...
    self.cancelBtn.frame = CGRectMake(10 + inset.left, inset.top+10, GetMatchValue(ML(@"取消"), 16, YES, 32), 32);
    CGFloat doneBtnW = GetMatchValue(ML(@"完成"), 16, YES, 32);
    self.doneBtn.frame = CGRectMake(kViewWidth - doneBtnW - inset.right - 10, inset.top + 10, doneBtnW, 32);
    ...
    if (_type & ZLImageEditTypeFilter) {
...
- (void)setupUI
{
    ...
    [self.cancelBtn setTitle:ML(@"取消") forState:UIControlStateNormal];
    ...
    [self.doneBtn setTitle:ML(@"完成") forState:UIControlStateNormal];
    [self.doneBtn setTitleColor:GC.MC forState:UIControlStateNormal];
    ...
    
    {
        if (_type & ZLImageEditTypeClip) {
            ...
            [self.clipBtn setImage:_configuration.clipImage forState:UIControlStateNormal];
        ...
        if (_type & ZLImageEditTypeRotate) {
            ...
            [self.rotateBtn setImage:_configuration.rotateImage forState:UIControlStateNormal];
        ...
        if (_type & ZLImageEditTypeFilter) {
            ...
            [self.filterBtn setImage:_configuration.filterImage forState:UIControlStateNormal];
        ...
        if (_type & ZLImageEditTypeDraw) {
            ...
            [self.drawBtn setImage:_configuration.drawImage forState:UIControlStateNormal];
...
- (NSInteger)toolCount
{
    NSArray *arr = @[@(ZLImageEditTypeClip), @(ZLImageEditTypeRotate), @(ZLImageEditTypeFilter), @(ZLImageEditTypeDraw), @(ZLImageEditTypeMosaic)];
    NSInteger count = 0;
...
- (void)setupFilterMenu
{
    ...
    _filterMenu.backgroundColor = [UIColor clearColor];
    _filterMenu.showsHorizontalScrollIndicator = NO;
    _filterMenu.clipsToBounds = NO;
    [self addSubview:_filterMenu];
    ...
    for (int i = ZLFilterTypeOriginal; i <= ZLFilterTypeColorInvert; i++) {
        ZLFilterItem *item = [[ZLFilterItem alloc] initWithFrame:CGRectMake(x, 0, W, H) Image:iconImage filterType:i target:self action:@selector(tapFilter:)];
...
- (void)setupDrawMenu
{
    ...
    [self.revokeBtn setImage:_configuration.revokeImage forState:UIControlStateNormal];
...
- (void)setupClipMenu
{
    ...
    if (!self.isFixedRatio) {
        ...
        [_rotateRatioBtn setBackgroundImage:_configuration.btnRotateImage forState:UIControlStateNormal];
...
{
    if (type & ZLImageEditTypeClip) {
        if (self.isFixedRatio) {
            
            if (!self.ishadFixedRatio) {
                self.ishadFixedRatio = YES;
            }else {
                return;
            }
        }
...
- (void)tapFilter:(UITapGestureRecognizer *)sender
{
    ...
    //加滤镜
    self.imageView.image = [ZLFilterTool filterImage:_editImage filterType:item.filterType];
}

- (void)setSelectFilterItem:(ZLFilterItem *)selectFilterItem
{
