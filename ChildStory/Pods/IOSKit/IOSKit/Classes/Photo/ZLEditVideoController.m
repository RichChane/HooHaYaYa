...
- (instancetype)initWithLeft:(UIImage *)l_ Right:(UIImage *)r_
{
    self = [super init];
    if (self) {
        leftIcom = l_;
...
- (void)setupUI
{
    ...
    _leftView = [[UIImageView alloc] initWithImage:leftIcom];
    ...
    _rightView = [[UIImageView alloc] initWithImage:rightIcom];
...
- (void)viewDidLayoutSubviews
{
    ...
    _cancelBtn.frame = CGRectMake(10 + inset.left, 7, GetMatchValue(ML(@"取消"), 16, YES, bottomBtnH), bottomBtnH);
...
- (void)setupUI
{
    ...
    self.editView = [[ZLEditFrameView alloc] initWithLeft:configuration.videoLeftImage Right:configuration.videoRightImage];
...
- (void)creatBottomView
{
    ...
    [_cancelBtn setTitle:ML(@"取消") forState:UIControlStateNormal];
    ...
    [_doneBtn setTitle:ML(@"完成") forState:UIControlStateNormal];
    [_doneBtn setBackgroundColor:GC.MC];
    [_doneBtn setTitleColor:GC.CWhite forState:UIControlStateNormal];
...
- (void)btnDone_click
{
    ...
    [ZLPhotoManager exportEditVideoForAsset:_avAsset range:[self getTimeRange] type:configuration.exportVideoType complete:^(BOOL isSuc, PHAsset *asset) {
        [hud hide];
        if (isSuc) {
            ...
            if (strongNav.callSelectVideoBlock && model) {
                strongNav.callSelectVideoBlock(model);
            }
        } else {
            zl_strongify(weakSelf);
            [strongSelf startTimer];
            ShowToastLong(@"视频编辑失败");
        }
    }];
}
