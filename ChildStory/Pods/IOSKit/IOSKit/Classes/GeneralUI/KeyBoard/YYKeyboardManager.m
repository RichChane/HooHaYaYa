...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        if (trans.toFrame.origin.y == SCREEN_HEIGHT) {
            [self performSelector:@selector(keyboardChange:) withObject:nil afterDelay:0 inModes:@[NSRunLoopCommonModes]];
//        todoSomething
        }else{
//            [[self class] cancelPreviousPerformRequestsWithTarget:self selector:@selector(todoSomething:) object:nil];
            [[self class] cancelPreviousPerformRequestsWithTarget:self];
            [self performSelector:@selector(keyboardChange:) withObject:nil afterDelay:0.3 inModes:@[NSRunLoopCommonModes]];
...
- (void)keyboardChange:(id)sender
{
    ...
    for (id<YYKeyboardObserver> observer in _observers.copy) {
        if ([observer respondsToSelector:@selector(keyboardChangedWithTransition:)]) {
            [observer keyboardChangedWithTransition:_trans];
        }
    }
}
