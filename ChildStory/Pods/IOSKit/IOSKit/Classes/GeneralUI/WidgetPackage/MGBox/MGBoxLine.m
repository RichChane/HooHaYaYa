...
#define DEFAULT_LINE_PADDING 15
#define DEFAULT_ITEM_PADDING 0
...
//            constrainedToSize:CGSizeMake(width - 24, 480)];
    ...
    CGSize fsize = [label.text boundingRectWithSize:CGSizeMake(width - 24, 480) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
...
+ (id)lineWithLeft:(NSObject *)left right:(NSObject *)right size:(CGSize)size{
    MGBoxLine *line = [[self alloc] initWithFrame:CGRectMake(0, 0,size.width,size.height)];
...
- (void)setUnderlineType:(UnderlineType)type {
    ...
    case MGUnderlineTop:
        self.solidUnderline.frame = CGRectMake(15, 0, size.width, OnePX);
        [self.layer addSublayer:self.solidUnderline];
        break;
    case MGUnderlineBottom:
        self.solidUnderline.frame =
                CGRectMake(15, size.height - OnePX, size.width, OnePX);
        [self.layer addSublayer:self.solidUnderline];
        break;
