...
-(void)setValueArr:(NSArray<NSArray *> *)valueArr{
    ...
    for (NSArray *arr in _valueArr) {
        ...
        for (id number in arr) {
            ...
            if ([number isKindOfClass:[NSNumber class]] && ((NSNumber *)number).integerValue > max) {
                max = ((NSNumber *)number).integerValue;
            }else if ([number isKindOfClass:[NSString class]] && ((NSString *)number).integerValue > max){
                max = ((NSString *)number).integerValue;
            }
        }
...
-(void)showAnimation{
    ...
    if (_needXandYLine) {
        ...
        for (NSInteger i = 0; i < 3; i++) {
            ...
            textLayer.frame = CGRectMake(0, CGRectGetHeight(self.frame) - _originSize.y -height - 5, _originSize.x - 3, 15);
            ...
            if ((i + 1) * pace < 10000) {
                textLayer.string = [NSString stringWithFormat:@"%ld",(i + 1) * pace];
            }else{
                textLayer.string = [NSString stringWithFormat:@"%.1lfw",(i + 1) * pace /10000.0];
            }
        ...
        [shapeLayer setLineDashPattern:@[@(1),@(6)]];
    ...
    if (_xShowInfoText.count == _valueArr.count && _xShowInfoText.count > 0) {
        ...
        for (NSInteger i = 0; i<_xShowInfoText.count; i++) {
            ...
            CGFloat wid =  _columnWidth + _typeSpace;
            ...
            textLayer.frame = CGRectMake( (i + 1) * wid + _drawFromOriginX, CGRectGetHeight(self.frame) - _originSize.y + 5, wid, size.height);
    ...
    if (_isColumnSerial) {
        ...
        for (NSInteger i = 0; i<_valueArr.count; i++) {
            ...
            UIButton *itemBtn = [[UIButton alloc] initWithFrame:CGRectMake((i + 1) * (_columnWidth + _typeSpace) + _drawFromOriginX, _originSize.y - 20, _columnWidth + _typeSpace, CGRectGetHeight(self.frame) - _originSize.y - 1)];
            itemBtn.tag = i + 1;
            ...
            for (NSInteger j = 0; j<arr.count; j++) {
                ...
                CGRect itemFrame = CGRectMake(_typeSpace / 2 , itemBtn.frame.size.height, _columnWidth, 0);
                UIView *itemsView = [[UIView alloc] initWithFrame:itemFrame];
                ...
                [UIView animateWithDuration:1 animations:^{
                    itemsView.frame = itemFrame;;
                    
                } completion:^(BOOL finished) {
                    
                }];
                
                [itemBtn addSubview:itemsView];
            }
        }
    }else{
        /*        动画展示   并列      */
...
//////////// 回调
- (void)selectToDo:(UIButton *)btn{
    if (_columnChartDelegate && [_columnChartDelegate respondsToSelector:@selector(selectColumnTag:Point:)]) {
        NSInteger p_x = btn.frame.size.width / 2 + btn.frame.origin.x;
        if (p_x + 50 > self.frame.size.width) {
            p_x = self.frame.size.width - 50;
        }
        
        [_columnChartDelegate selectColumnTag:(btn.tag - 1) Point:CGPointMake(p_x, 30)];
    }
}
