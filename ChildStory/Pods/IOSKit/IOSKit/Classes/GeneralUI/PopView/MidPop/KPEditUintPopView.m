...
#import <IOSKit/StringCheckout.h>
...
- (instancetype)initWithTitle:(NSString*)title unitTitle:(NSString*)unitTitle number:(double)number baseUnitTitle:(NSString*)baseUnitTitle isDefaultUnit:(BOOL)isDefaultUnit cancelBtnTitle:(NSString*)cancelBtnTitle okBtnTitle:(NSString*)okBtnTitle  makeSure:(ConfirmV3)makeSure cancel:(Cancel)cancel delete:(Delete)deleteAction
{
    ...
    if (self) {
        ...
        _number = [GeneralUse StrmethodComma:[NSString stringWithFormat:@"%f",number] FloatingNumber:2];
        if (number == 0) {
            _number = @"";
        }
        kMaxLength = 8;
...
- (void)createItems
{
    ...
    cancelControl.sd_layout
    .centerXEqualToView(baseUnitLabel)
    .centerYEqualToView(_titleLabel)
    .widthIs(cancelControl.width)
    .heightIs(cancelControl.height);
    ...
    UILabel *defualtLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(defualtBtn.frame) + 15,nowY, 150, defualtBtn.height)];
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([textField isEqual:_unitTextField]) {
        ...
        if (textField.text.length == 1 && ![string isEqualToString:@"."]) {
            NSString *firstText = [textField.text substringToIndex:1];
            if ([firstText isEqualToString:@"0"]) {
                textField.text = [firstText substringFromIndex:1];
            }
        }
        return [StringCheckout validateNumber:string text:textField.text floatCount:2];
    }
...
- (void)clickSureBtn:(UIButton *)sender
{
    ...
    if (self.confirmV3) {
        self.confirmV3(_unitTextField.text,[_unitNumer.text stringByReplacingOccurrencesOfString:@"," withString:@""].doubleValue,_isDefaultUnit);
    }
