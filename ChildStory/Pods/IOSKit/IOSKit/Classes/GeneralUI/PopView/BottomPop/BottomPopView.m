...
- (void)createContentView
{
    ...
    CGFloat ContentViewHeight = selectGroupViewHeight + ItemHeight + 10;
    UIView *contentView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, ContentViewHeight)];
    _contentView = contentView;
    contentView.backgroundColor = kUIColorFromRGB(0xF7F7F7);
    [self addSubview:contentView];
    
    CGFloat itemOriginY = 0.0;
    if (self.bottomPopType == KPBottomPopTypeNormal) {
        
        UIButton *cancelBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, selectGroupViewHeight + 10, SCREEN_WIDTH, ItemHeight)];
        cancelBtn.backgroundColor = GC.CWhite;
        ...
        UIView *topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, ItemHeight+10)];
        topView.backgroundColor = GC.CWhite;
    ...
    for (int i = 0; i < _selectArr.count ; i ++) {
        ...
        if (self.bottomPopType == KPBottomPopTypeNormal) {
            ...
            //CGSize titleSize = [itemTitle sizeWithFont:FontSize(17) maxSize:CGSizeMake(SCREEN_WIDTH-15*2, ItemHeight)];
            UILabel *itemLabel = [UIFactory createLabelWithText:itemTitle textColor:GC.CBlack font:FontSize(17)];
            itemLabel.frame = CGRectMake(15, 0, SCREEN_WIDTH-15*2, ItemHeight);
            [itemBtn addSubview:itemLabel];
            
            if (_selectIndex == i) {
                UIImageView * imageView = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 39, 18, 24, 24)];
                imageView.image = ImageName(@"common_check");
        ...
        if (i < _selectArr.count - 1) {
            ...
            line.backgroundColor = GC.LINE;
            line.frame = CGRectMake(0, itemBtn.frame.size.height-OnePX, itemBtn.frame.size.width, OnePX);
