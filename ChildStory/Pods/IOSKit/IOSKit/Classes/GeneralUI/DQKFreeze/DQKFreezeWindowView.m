...
// ** frame 二维表大小 freezePoint 三个维度的结点 cellViewSize cell的大小 */
        ...
        if (self.keyIndexPath != nil) {
            ...
            if ([sectionViewCell superview]) {
                ...
                if (cellAtSection == self.keyIndexPath.section) {
                    ...
                    if (self.delegate && [self.delegate respondsToSelector:@selector(sectionCellReachKey:withSection:)]) {
...
- (void)tapMainViewCell:(UITapGestureRecognizer *) tapGestureRecognizer {
    NSInteger row = tapGestureRecognizer.view.frame.origin.y / self.cellViewSize.height;
    CGFloat section = tapGestureRecognizer.view.frame.origin.x / self.cellViewSize.width;
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:row inSection:ceil(section)];
    [_delegate freezeWindowView:self didSelectIndexPath:indexPath];
}

- (void)tapRowViewCell:(UITapGestureRecognizer *) tapGestureRecognizer
{
    NSInteger row = tapGestureRecognizer.view.frame.origin.y / self.cellViewSize.height;
    CGFloat section = tapGestureRecognizer.view.frame.origin.x / self.cellViewSize.width;
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:row inSection:ceil(section)];
    if (self.delegate && [self.delegate respondsToSelector:@selector(freezeWindowView:didSelectRow:)]) {
        [_delegate freezeWindowView:self didSelectRow:row];
    }
...
- (void)refreshViewWhenScroll {
    ...
    BOOL isRemovesectionMax = YES;
    if (sectionMax >= [_dataSource numberOfSectionsInFreezeWindowView:self] - 1) {
        sectionMax = [_dataSource numberOfSectionsInFreezeWindowView:self] - 1;
        isRemovesectionMax = NO;
    }
    ...
    BOOL isRemoverowMax = YES;
    if (rowMax >= [_dataSource numberOfRowsInFreezeWindowView:self] - 1) {
...
- (void)removeSectionViewCellWithSection:(NSInteger)section {
    if (section < 0 || section > _sectionNumber - 1) {
        return;
    }
...
- (void)addRowViewCellWithRow:(NSInteger)row {
    ...
    if (rowViewCell != nil && [rowViewCell superview] == nil) {
        ...
        if ([self dequeueReusableRowCellWithIdentifier:rowReuseIdentifier forRow:row] == nil) {
            if (_delegate && [_delegate respondsToSelector:@selector(freezeWindowView:cellAtRow:)]) {
                UITapGestureRecognizer *gestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapRowViewCell:)];
