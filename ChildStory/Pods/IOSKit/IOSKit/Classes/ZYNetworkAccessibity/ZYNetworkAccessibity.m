...
- (void)startCheck {
    if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self currentReachable]) {
        ...
        [self notiWithAccessibleState:ZYNetworkAccessible AndZYNetworkType:ZYNetworkTypeUnknown];
        return;
    }
        ...
        case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(ZYNetworkType type) {
                ...
                if (type == ZYNetworkTypeCellularData || type == ZYNetworkTypeWiFi) {
                    [self notiWithAccessibleState:ZYNetworkRestricted AndZYNetworkType:type];
                } else {  // 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:ZYNetworkUnknown AndZYNetworkType:ZYNetworkTypeUnknown];
                }
            }];
...
- (void)notiWithAccessibleState:(ZYNetworkAccessibleState)state AndZYNetworkType:(ZYNetworkType)type {
    if (_automaticallyAlert) {
        if (state == ZYNetworkRestricted) {
            [self showNetworkRestrictedAlertZYNetworkType:type];
        } else {
            [self hideNetworkRestrictedAlert];
        }
        _automaticallyAlert = NO;
    }
...
- (void)showNetworkRestrictedAlertZYNetworkType:(ZYNetworkType)type {
    if (!_alertController || (_alertController.presentingViewController == nil && ![_alertController isBeingPresented])) {
        
        NSString * titleStr = @"";
        NSString * contentStr = @"";
        
        if (type == ZYNetworkTypeCellularData) {
            titleStr = [NSString stringWithFormat:@"%@“%@”%@",ML(@"已为"),[DeviceBaseData appName],ML(@"关闭蜂窝移动数据")];
            contentStr = ML(@"您可以在“设置”中为此应用打开蜂窝移动数据");
        }else if (type == ZYNetworkTypeWiFi) {
            titleStr = [NSString stringWithFormat:@"%@“%@”%@",ML(@"已为"),[DeviceBaseData appName],ML(@"关闭无限局域网")];
            contentStr = ML(@"您可以在“设置”中为此应用打开无线局域网");
        }else {
            titleStr = ML(@"网络连接失败");
            contentStr = ML(@"检测到网络权限可能未开启，您可以在“设置”中检查蜂窝移动网络");
        }
        
        _alertController = [UIAlertController alertControllerWithTitle:titleStr message:contentStr preferredStyle:UIAlertControllerStyleAlert];
        
        [_alertController addAction:[UIAlertAction actionWithTitle:ML(@"设置") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [_alertController addAction:[UIAlertAction actionWithTitle:ML(@"好") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self hideNetworkRestrictedAlert];
        }]];
        
        [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:_alertController animated:YES completion:nil];
    }
}
